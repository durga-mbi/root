...
export const createPlanPurchase = async (
  ...
  return prisma.$transaction(async (tx) => {
    ...
    if (isAutoApproval && data.purchase_type !== "SHARE_PURCHASE") {
      // 🔹 Credit DP to user wallet on auto-approval
      await updateWalletDP(userId, Number(data.dp_amount), tx);

      // 🔹 Process royalty qualification if sponsor exists on auto-approval
...
import { approvePlanPurchase, rejectPlanPurchase } from "../admin/planPurchaseApproval.useCase";

export const approvePurchase = async (purchaseId: number, adminId: number) => {
  return approvePlanPurchase(purchaseId, adminId);
};

export const rejectPurchase = async (purchaseId: number, adminId: number) => {
  return rejectPlanPurchase(purchaseId, adminId);
};
