 ...
 return KycRepository.runInTransaction(async (tx) => {
    ...
    if (existingKyc && (existingKyc.status === KycStatus.PENDING || existingKyc.status === KycStatus.APPROVED)) {
      throw AppError.conflict(
        `KYC already ${existingKyc.status.toLowerCase()}.`
      );
    }
...
export const updateKyc = async (
  ...
  if (latest.status === KycStatus.PENDING) {
    throw AppError.forbidden(
      "KYC is under review and cannot be updated"
    );
  }

  // If status is REJECT, we create a NEW record for history instead of updating the old one
  return createKyc(userId, data);
};
