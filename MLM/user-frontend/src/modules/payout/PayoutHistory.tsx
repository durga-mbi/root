...
import {
    ...
    TableContainer, TableHead, TableRow, Collapse, Paper, Button, Stack, Step, Stepper, StepLabel
} from '@mui/material';
import { 
    Eye, 
    EyeOff, 
...
function PayoutRow({ row }: { row: any }) {
                                ...
                                <Grid size={{ xs: 12, md: 7 }}>
                                    ...
                                    <Stack spacing={2}>
                                        ...
                                        <Box display="flex" justifyContent="space-between" p={1.5} sx={{ bgcolor: '#f8fafc', borderRadius: 2 }}>
                                            ...
                                            <Typography variant="body2" fontWeight={700}>REF#{row.payout?.payoutCycle || 'N/A'}</Typography>
                                        </Box>
...
const SUMMARY_CARDS = (data: any) => [
    { label: 'Total Withdrawn', value: `₹${Number(data?.data?.reduce((acc: number, cur: any) => acc + Number(cur.netAmount), 0) || 0).toFixed(2)}`, color: designConfig.colors.primary.main, icon: <Wallet size={20} /> },
    { label: 'Pending Payouts', value: `₹0.00`, color: designConfig.colors.warning.main, icon: <Info size={20} /> },
    { label: 'Total Service Fees', value: `₹${Number(data?.data?.reduce((acc: number, cur: any) => acc + Number(cur.adminCharges), 0) || 0).toFixed(2)}`, color: designConfig.colors.error.main, icon: <Receipt size={20} /> },
];
...
export default function PayoutHistory() {
    ...
    return (
        <Box sx={{ p: { xs: 2, md: 4 }, background: '#f8fafc', minHeight: '100vh' }}>
            <Box mb={4}>
                <Typography variant="h4" fontWeight={900} sx={{ color: '#1e293b' }}>Payout History</Typography>
                <Typography variant="body2" color="text.secondary">View your bank transfer records and disbursement timelines</Typography>
            </Box>
            ...
            <Typography variant="h6" fontWeight={800} mb={3} display="flex" alignItems="center" gap={1}>
                <Receipt size={20} /> Bank Disbursement Statement
            </Typography>
